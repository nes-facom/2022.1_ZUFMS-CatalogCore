...
```bash
make gen_sql_schema > infra/database/postgres/docker-entrypoint-initdb.d/1schema.sql
```

#### Geração da seed para popular o banco com os scopes
```bash
make gen_sql_scopes > infra/database/postgres/docker-entrypoint-initdb.d/5seed_scopes.sql
```