...
<template>
  <main :class="`w-max h-full flex flex-col ${
    props.showTermclassNavigation && 'overflow-y-hidden'
  }`" ref="mainRef">
    ...
    <header class="flex mt-8 mb-5">
      <TermInfo v-for="term in termsList" :key="term.name" :title="term.title" :description="term.description" :class="
    ...
    <Spreadsheet class="h-full" :terms="terms" :submissionMode="props.submissionMode" :currentTermclass="
      props.showTermclassNavigation ? currentTermclass.name : undefined
    " @scrollY="onSpreadsheetScrollY" @changeTermclass="changeTermclass" />
  </main>
</template>
